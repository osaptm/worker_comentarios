...
async function guardar_comentarios(array_comentarios, page) {
  console.log("Dentro de guardar_comentarios");
  let nro_coment = 0;
  for await (const comentario of array_comentarios) {
    nro_coment++; console.log("Dentro For - comentario NRO "+nro_coment);
    ...
    if(boton_titulo.includes('Google')){
      ...
      try {
        ...
        console.log("Error Caja Comentario Traducido"); 
        continue;      
      }

      const caja_comentario = await page.$('.HyAcm > .QFfSR > .WMIKb');
      console.log("Traducido OK ...")
      ...
      const usuario = await caja_comentario.$('.mwPje >.XExLl > .zpDvc > span');
            ...
            if (usuario) {
              ...
              if(txt_mensaje.length > 0){
                console.log("Guardamos comentario ...")
        ...
        console.log("Clic en cerrar caja ...")
...
async function trae_comentarios(page) {
  ...
  if (comentarios) {
    ...
    if(array_comentarios.length > 0){
      ...
      try {
        await guardar_comentarios(array_comentarios, page);
      } catch (error) {
        console.log(`Error guardar_comentarios `+ error)        
      }
      
    }else{
      console.log(`-----> RAROOOOOO NO HAY COMENTARIOS`)
    }
...
(async () => {
  try {    
    ...
    if (filtros) {
      ...
      await new Promise(r => setTimeout(r, 1000));
      console.log("Boton idiomas -> "+title_idiomas);
      ...
      try { 
        ...
        throw (`-----> ERROR CLICK TODOS LOS IDIOMAS - ${workerData.url}`) 
      }
      ...
      console.log("Boton -> "+title_todos_idiomas);
