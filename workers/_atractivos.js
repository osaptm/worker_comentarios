...
async function get_seccion_fotos(page) {
  ...
  if(obj_seccion_fotos.datos.length === 0){ 
    console.log("Otra Galeria");
    const seccion_fotos_ = await page.$(".IdURT > div > ul");
    if (seccion_fotos_) {
        console.log("Dentro de Otra Galeria");
          const array_fotos = await seccion_fotos_.$$("li");
        if (array_fotos.length !== 0) {
          console.log("Tenemos Array de Fotos ", array_fotos.length);
            for (const htmlFoto of array_fotos) {
              const foto = await htmlFoto.$("picture > img");
              if(!foto) continue;
              let urlFoto = '';
              try {                  
                urlFoto = await foto.evaluate(el => el.getAttribute('src'));
              } catch (error) {
                urlFoto = ''
                continue;
              }
              urlFoto = urlFoto.replace("&s=1", "");
              if(urlFoto !== "" && urlFoto.includes("/media/photo") && !urlFoto.includes("50x50")){
                console.log(urlFoto);
                obj_seccion_fotos.datos.push(urlFoto);
              }
